...
def create_phone():
    ...
    if not phone_value.isdigit():
        ...
        return 'Error: The number must be wrong. Please check & try again.'
...
def read_phone():
    ...
    # /phones/read/?order_by=contactName
    contact_name = request.args['order_by']

    cur = con.cursor()
    sql_query = f'''
    SELECT * FROM phones ORDER BY {contact_name} DESC;
    '''
    cur.execute(sql_query)
    result = cur.fetchall()
    con.close()
...
def update_phone():
    import sqlite3
    # http://127.0.0.1:5000/phones/update/?contactName=Saroshi&Phone=380638732561
